...
export default function WeatherWithAutocompleteResponsive() {
  ...
  const suggestionsController = useRef(null);
  const weatherController = useRef(null);
  ...
  /**
   * Fetch weather (current + forecast) for a city
   */
  ...
  /**
   * Process hourly forecast (next 8 periods)
   */
    ...
    >
          ...
          onChange={(e, newVal) => {
            ...
            if (typeof newVal === "string") {
              ...
              fetchSuggestions(newVal).then?.(() => {
                ...
                if (cached && cached.length > 0) fetchWeatherForCity(cached[0]);
              });
            } else {
      ...
      >
        {loadingWeather ? (
          ...
          <Typography>Search for a city or choose one</Typography>
        ) : (
          <>
            {/* Current Weather */}
